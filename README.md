...
- **平台相关代码隔离**：两端都用 `_windows.go` / `_other.go` 后缀 + `//go:build` 构建约束
  隔离平台差异，非 Windows 平台仍可编译（降级为管道 shell、内嵌终端），便于开发自测。
...
- **操作系统**：主要面向 **Windows**（伪终端、弹窗、控制台 raw 模式均为 Windows 实现）；
  两端在 Linux / macOS 上也可编译（功能降级：管道 shell + 内嵌终端），可用于开发自测。
- **网络依赖**（首次编译会自动拉取）：
