...
func handleBot(out io.Writer, sc *bufio.Scanner, m *Manager, attachSrv *AttachServer, id string) {
	...
	title := "RemoteAssist - " + b.Title()
	if err := openAttachWindow(title, id, attachSrv.Addr(), attachSrv.Token()); err != nil {
...
func handleBots(out io.Writer, args []string, m *Manager, botslog *logx.BotsLog) {
	if len(args) == 0 {
		...
		fmt.Fprintf(out, "%-12s %-16s %-34s %s\n", "BOTID", "名称", "系统", "地址")
		for _, b := range list {
			fmt.Fprintf(out, "%-12s %-16s %-34s %s\n",
				b.ID, truncRunes(b.NameOrDash(), 15), truncRunes(b.OSOrDash(), 33), b.Addr())
		}
		return
	}
	switch args[0] {
...
// truncRunes 按 rune 数截断字符串（中文等宽显示时防止超长名称撑乱表格）
